...
def rnn_backward(dh, cache):
  ...
  for t in reversed(xrange(T)):
    ...
    if t > 0:
      ...
      dh0 = dt[1]
    dWx, dWh, db = dWx + dt[2], dWh + dt[3], db + dt[4]
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  dnext_c = np.copy(dnext_c)
  dnext_c += dnext_h * o * (1 - np.tanh(next_c)**2)
  di, df = dnext_c * g, dnext_c * prev_c
  do, dg = dnext_h * np.tanh(next_c), dnext_c * i
  dai, daf = di * i * (1 - i), df * f * (1 - f)
  dao, dag = do * o * (1 - o), dg * (1 - g**2)
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  h[:, -1, :] = h0
  for t in xrange(T):
    h[:, t, :], c[:, t, :], _ = lstm_step_forward(x[:, t, :], h[:, t - 1, :],
                                                  c[:, t - 1, :], Wx, Wh, b)
  cache = (h, c, x, h0, Wx, Wh, b)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_backward(dh, cache):
  ...
  T = dh.shape[1]
  for t in reversed(xrange(T)):
    dt = lstm_step_backward(dh[:, t, :], dc[:, t, :],
                           (x[:, t, :],
                            h[:, t - 1, :] if t > 0 else h0,
    ...
    if t > 0:
      ...
      dh0 = dt[1]
    dWx, dWh, db = dWx + dt[3], dWh + dt[4], db + dt[5]
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
